...
function GameInner() {
  ...
  // Handlers ESTÁVEIS (useCallback): sem isto, cada render do pai gera
  // um onDone novo, e as fases que têm onDone nas deps do useEffect
  // re-disparam a narração — o que parece "a tela voltando sozinha".
  const goIntroPhon = useCallback(() => setStage("intro-phon"), []);
  const goPhonemic = useCallback(() => setStage("phonemic"), []);
  const goIntroDisc = useCallback(() => setStage("intro-disc"), []);
  const goDiscovery = useCallback(() => setStage("discovery"), []);
  const goIntroBattle = useCallback(() => setStage("intro-battle"), []);
  const goBattle = useCallback(() => setStage("battle"), []);
  const goIntroRead = useCallback(() => setStage("intro-read"), []);
  const goReading = useCallback(() => setStage("reading"), []);
  const goIntroWrite = useCallback(() => setStage("intro-write"), []);
  const goWriting = useCallback(() => setStage("writing"), []);
  const goVictory = useCallback(() => setStage("victory"), []);
  ...
  return (
    <div>
      ...
      {stage === "checkin" && (
        <EmotionalCheckin hero={hero} onDone={goIntroPhon} />
      )}
      ...
      {stage === "phonemic" &&
        (isDay1 ? (
          <VowelAdventure hero={hero} onDone={goIntroBattle} />
        ) : (
          <PhonemicPhase hero={hero} plan={plan} onDone={goIntroDisc} />
        ))}
      ...
      {stage === "discovery" && (
        <Discovery hero={hero} plan={plan} onDone={goIntroBattle} />
      )}
      ...
      {stage === "battle" && (
        <MonsterBattle hero={hero} plan={plan} onDone={goIntroRead} />
      )}
      ...
      {stage === "reading" && (
        <ReadingPhase hero={hero} plan={plan} onDone={goIntroWrite} />
      )}
      ...
      {stage === "writing" && (
        <Writing hero={hero} plan={plan} onDone={goVictory} />
      )}
